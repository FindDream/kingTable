(function ($) {
    ...
    KingTable.defaults={
        ...
        groupNum:1,                                 //分页-当前组
        search:false,                               //搜索-是否开启搜索
        bodyForSearchBackup:[],                    //搜索-备份的tbody数据
        sort:false,                                 //排序-是否开启排序
    ...
    KingTable.prototype.pagination = function(tdCell){
        ...
        if(this.options.currentPageNum >= 2){
            var oA = $("<a/>");
            oA.attr("href","#"+(this.options.currentPageNum - 1));
            oA.html("上一页");
            $td.append(oA);
        }
        //普通显示格式
        ...
        if(!!this.options.foreAndAft){
            //尾页
            var oA = $("<a/>");
            oA.attr("href","#" + this.options.maxPageNum);
            oA.html("尾页");
            $td.append(oA);
        }
        if(!!this.options.pageNumTip){
            //提示
            var oSpan = $("<span/>");
            oSpan.html(this.options.currentPageNum+"页/"+this.options.maxPageNum+"页");
            $td.append(oSpan);
        }
